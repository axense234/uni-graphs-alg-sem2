...
std::pair<std::map<Vertex, EdgeCost>, std::map<Vertex, Vertex>> ADTDirectedGraph::dijkstraAlgorithm(ADTDirectedGraph graph, Vertex start, Vertex end)
{
    ...
    std::priority_queue<Vertex, std::vector<Vertex>, decltype(vertexComparison)> minHeapQueue(vertexComparison);
    ...
    while (!minHeapQueue.empty() && !found)
    {
        // those 2 lines are equivalent to the q.enqueue operation
        Vertex x = minHeapQueue.top();
        minHeapQueue.pop();
        ...
        while (neighOutBegin != neighOutEnd)
        {
            ...
            if (dist.count(y) == 0 || (dist.at(x) + graph.costs.at(Edge{x, y})) < dist[y])
            {
                dist[y] = dist[x] + graph.costs.at(Edge{x, y});
                minHeapQueue.push(y);
                prev[y] = x;
            }
...
std::pair<bool, std::vector<Vertex>> ADTDirectedGraph::topologicalSort(ADTDirectedGraph graph)
{
    std::vector<Vertex> sorted{};
    std::queue<Vertex> queue{};
    std::map<Vertex, unsigned int> count{};

    // we want to build our count map where in the queue we push our vertices without predecessors
    ADTDirectedGraphIterator it = graph.parseVertices();
    it.first();
    while (it.valid())
    {
        Vertex x = it.getCurrent();
        count[x] = graph.getInDegree(x);
        if (count[x] == 0)
        {
            queue.push(x);
        }
    ...
    // we use our queue to hold our vertices without predecessors and we slowly sort them by modifying our count indegrees
    while (!queue.empty())
    {
        Vertex x = queue.front();
        queue.pop();
    ...
    // if we couldnt access all the vertices in the graph by our alg it means the graph has a cycle
    if (sorted.size() < graph.nbVertices())
    {
        return {false, sorted};
    }
