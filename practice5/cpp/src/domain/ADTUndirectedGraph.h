...
public:
    ...
    /**
     * @brief Finds and returns a hamiltonian cycle in the given graph or throws HamCycleException if it cannot find one as such
     *
     * @param graph the undirected graph
     * @exception HamCycleException if given graph is empty
     * @exception HamCycleException if given graph doesnt contain a hamiltonian cycle
     * @return the hamiltonian cycle of type std::vector<Vertex>
     */
    static std::vector<Vertex> findHamiltonianCycle(ADTUndirectedGraph &graph) noexcept(false);

    // utility recursive function for findHamiltonianCycle
    bool hamiltonianCycleBT(Vertex current,
